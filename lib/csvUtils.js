...
export const importRepresentativesFromCSV = async (mappedData, userId) => {
  // Cache to store companies found/created during this import session
  ...
  const representatives = await Promise.all(mappedData.map(async (row) => {
    ...
    if (row.company_name && row.company_name.trim()) {
      ...
      // Check if we already found/created this company in this import session
      if (companyCache.has(companyName.toLowerCase())) {
        repData.company_id = companyCache.get(companyName.toLowerCase());
      } else {
        // First, try to find existing company (exact case-insensitive match)
        ...
        // Cache the company ID for this import session
        if (companyData) {
          companyCache.set(companyName.toLowerCase(), companyData.id);
          repData.company_id = companyData.id;
        } else {
