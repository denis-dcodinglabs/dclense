...
import { updateCandidate, uploadCV, getCVPublicUrl, removeCandidateCV } from '../app/candidates/candidatesHelpers';
...
export default function EditCandidateModal({ open, onClose, candidate, onCandidateUpdated }) {
  ...
  useEffect(() => {
    if (candidate && open) {
      setForm({
        ...
        date_available: candidate.date_available ? String(candidate.date_available).split('T')[0] : '',
        willing_to_relocate: candidate.willing_to_relocate || '',
        general_comments: candidate.general_comments || '',
        category: candidate.category || '',
        skills: candidate.skills || '',
        industry: candidate.industry || '',
        years_of_experience: candidate.years_of_experience || '',
        user_date_added: candidate.user_date_added ? String(candidate.user_date_added).split('T')[0] : '',
        cv: null,
      });
      setError('');
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Prepare fields that are not meant to be sent as-is
      delete updates.cv;

      // If a new CV is selected, upload and update cv_url
      if (form.cv) {
        const filePath = await uploadCV(form.cv);
        updates.cv_url = filePath;
      }
      ...
      // Reset form CV selection so the old CV block does not reappear
      setForm((f) => ({ ...f, cv: null }));
      onClose();
    } catch (err) {
      setError(err.message || 'Error updating candidate');
    } finally {
      setLoading(false);
    }
  };

  const handleRemoveExistingCV = async () => {
    if (!candidate?.id) return;
    setRemovingCV(true);
    setError('');
    try {
      const updated = await removeCandidateCV(candidate.id);
      if (onCandidateUpdated) onCandidateUpdated(updated);
      // After removal, ensure local candidate reference reflects no CV
      setForm((f) => ({ ...f, cv: null }));
    } catch (err) {
            ...
            <form className="space-y-4 px-6 py-4 flex-1 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 64px)' }} onSubmit={handleSubmit}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                ...
                <div className="flex">
                  ...
                  <Input name="date_available" type="date" value={form.date_available || ''} onChange={handleChange} className="w-full" />
                </div>
                <div className="flex">
                  <label className="block text-sm font-medium text-gray-700 mb-1">Willing to Relocate</label>
                  <select name="willing_to_relocate" value={form.willing_to_relocate || ''} onChange={handleChange} className="border rounded-md px-3 py-2 text-sm w-full">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                ...
                <div className="flex">
                  ...
                  <Input name="user_date_added" type="date" value={form.user_date_added || ''} onChange={handleChange} className="w-full" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">CV Document</label>
                {/* Existing CV information */}
                {candidate?.cv_url && !form.cv && (
                  <div className="flex items-center justify-between p-3 border border-gray-300 rounded-md bg-gray-50 mb-2">
                    <div className="flex-1">
                      <p className="text-sm font-medium text-gray-700 truncate">{candidate.cv_url.split('/').pop()}</p>
                      <Button
                        type="button"
                        variant="link"
                    ...
                    <div className="flex items-center gap-2">
                      <Button type="button" variant="outline" size="sm" onClick={handleRemoveExistingCV} disabled={removingCV}>
                ...
                {/* New CV selection or file input (only when there is no existing CV) */}
                {(!candidate?.cv_url || form.cv) && (
                  form.cv ? (
                    <div className="flex items-center gap-2 p-3 border border-gray-300 rounded-md bg-gray-50">
                      ...
                      <button type="button" onClick={handleRemoveCV} className="text-red-500 hover:text-red-700 text-lg font-bold focus:outline-none" aria-label="Remove CV">
                        ×
                      </button>
                    </div>
                  ) : (
                    <input type="file" name="cv" accept="application/pdf" onChange={handleChange} className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-medium file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" />
                  )
                )}
              </div>
              ...
              <div className="flex justify-end gap-2 mt-4">
                ...
                <Button type="submit" disabled={loading}>{loading ? 'Saving...' : 'Update Candidate'}</Button>
              </div>
            </form>
          </div>
        </div>
