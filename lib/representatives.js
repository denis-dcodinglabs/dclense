...
export const bulkAssignToMe = async (ids, userId) => {
  ...
  if (!error && data) {
    ...
    for (let i = 0; i < data.length; i++) {
      const oldRep = oldData?.find(old => old.id === data[i].id);
      await logAudit(userId, 'update', 'representatives', data[i].id, oldRep, data[i]);
    }
  }
