...
export const importRepresentativesFromCSV = async (mappedData, userId) => {
  ...
  const representatives = await Promise.all(mappedData.map(async (row) => {
    ...
    if (row.status) {
      ...
      // Trim whitespace and check if the status is valid
      const trimmedStatus = row.status.trim();
      if (validStatuses.includes(trimmedStatus)) {
        repData.status = trimmedStatus;
      } else {
        console.log('Invalid status found during representative import:', row.status, 'Setting to null');
        repData.status = null;
      }
    } else {
      repData.status = null;
