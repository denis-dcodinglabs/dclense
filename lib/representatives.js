...
import { getRepresentativesWithUserReadStatus, bulkSetRepresentativesReadStatus, markRepresentativeAsRead, getRepresentativesWithUserReadStatusAndFilteredCount } from './userReads';

export const getPaginatedRepresentatives = async (page = 1, limit = 50, filters = {}) => {
  if (filters.current_user_id) {
    const { data, count, error } = await getRepresentativesWithUserReadStatusAndFilteredCount(filters.current_user_id, page, limit, filters);
    return { data, count, error };
  }
  
  // Fallback for non-user-specific fetching
  return getRepresentatives(page, limit, filters);
};
