...
export const getRepresentatives = async (page = 1, limit = 50, filters = {}) => {
  ...
  // If we have data and need to filter by export status, do it after the main query
  let filteredData = data;
  if (data && filters.exported_filter && filters.user_id) {
    if (filters.exported_filter === 'exported_only') {
      // Get representatives that this user has exported
      const { data: exportedReps } = await supabase
        .from('user_exports')
        .select('representative_id')
        .eq('user_id', filters.user_id);
      
      const exportedIds = exportedReps?.map(e => e.representative_id) || [];
      filteredData = data.filter(rep => exportedIds.includes(rep.id));
    } else if (filters.exported_filter === 'not_exported_only') {
      // Get representatives that this user has NOT exported
      const { data: exportedReps } = await supabase
        .from('user_exports')
        .select('representative_id')
        .eq('user_id', filters.user_id);
...
export const getRepresentativeExportStatus = async (representativeIds, userId) => {
  ...
  const exportedIds = data?.map(e => e.representative_id) || [];
  return { data: exportedIds, error: null };
};

export const markRepresentativeAsExported = async (representativeId, userId) => {
  const { data, error } = await supabase
    .from('user_exports')
    .upsert([{
      user_id: userId,
...
export const markRepresentativesAsExported = async (representativeIds, userId) => {
  const exports = representativeIds.map(id => ({
    user_id: userId,
