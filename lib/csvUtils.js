...
export const importRepresentativesFromCSV = async (mappedData, userId) => {
  // Cache to store companies found/created during this import to avoid duplicates
  ...
  const representatives = await Promise.all(mappedData.map(async (row) => {
    ...
    if (row.company_name && row.company_name.trim()) {
      ...
      // Check if we already found/created this company in this import batch
      ...
      // If no exact match found, try exact case-insensitive match
      if (!companyData) {
      ...
      if (!companyData) {
        ...
        // Cache the company ID for this company name
        if (companyData?.id) {
