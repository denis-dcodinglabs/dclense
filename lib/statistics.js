...
export const getCreationStats = async (startDate, endDate) => {
  try {
    ...
    const filters = {};
    if (startDate) {
      filters.start = new Date(startDate + 'T00:00:00.000Z').toISOString();
    }
    if (endDate) {
...
export const getStatusCounts = async (startDate, endDate, category, statuses) => {
  try {
    // Build date filters
    const filters = {};
    if (startDate) {
      filters.start = new Date(startDate).toISOString().split('T')[0];
    }
    if (endDate) {
      filters.end = new Date(endDate).toISOString().split('T')[0];
    }

    let query;
    let tableName;

    if (category === 'Representatives') {
    ...
    if (error) {
      console.error(`Error fetching ${tableName} status counts:`, error);
      return { data: null, error };
    }
    ...
    const formattedCounts = Object.entries(statusCounts).map(([status, count]) => ({
      status,
      count,
    ...
    console.error('Unexpected error in getStatusCounts:', err);
    return { data: null, error: err };
  }
};
