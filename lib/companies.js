...
import { getCompaniesWithUserReadStatus, bulkSetCompaniesReadStatus, markCompanyAsRead, getCompaniesWithUserReadStatusAndFilteredCount } from './userReads';

export const getPaginatedCompanies = async (page = 1, limit = 50, filters = {}) => {
  if (filters.current_user_id) {
    const { data, count, error } = await getCompaniesWithUserReadStatusAndFilteredCount(filters.current_user_id, page, limit, filters);
    return { data, count, error };
  }
  
  // Fallback for non-user-specific fetching, though the app seems to always provide a user
  return getCompanies(page, limit, filters);
};
  ...
  const { data, error } = await supabase
    ...
    .select(`
      *,
      assigned_user:assigned_to(first_name, last_name),
      created_user:created_by(first_name, last_name),
      updated_user:updated_by(first_name, last_name),
      representatives(*)
    `)
    .eq('id', id)
    .eq('is_deleted', false)
