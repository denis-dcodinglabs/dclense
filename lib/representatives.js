...
export const getRepresentatives = async (page = 1, limit = 50, filters = {}) => {
  ...
  let query = supabase
    ...
    .select(`
      *,
      ...
      user_exports!left(user_id, is_exported, exported_at)
    `)
  ...
  if (filters.exported_filter === 'exported_only') {
    // Filter to only representatives that have been exported by current user
    if (currentUser) {
      // Use a subquery to find representatives exported by current user
      ...
      const exportedRepIds = exportedReps?.map(exp => exp.representative_id) || [];
      if (exportedRepIds.length > 0) {
        query = query.in('id', exportedRepIds);
      } else {
        // If no exports found, return empty result
        query = query.eq('id', '00000000-0000-0000-0000-000000000000');
      }
    }
  }
  if (filters.exported_filter === 'not_exported_only') {
    // Filter to only representatives that have NOT been exported by current user
    if (currentUser) {
      // Use a subquery to find representatives NOT exported by current user
      const { data: exportedReps } = await supabase
        .from('user_exports')
        .select('representative_id')
        .eq('user_id', currentUser)
