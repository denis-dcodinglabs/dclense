...
export const importCompaniesFromCSV = async (mappedData, userId) => {
  ...
  for (const [importedName, existingName] of duplicateNameToExisting.entries()) {
    ...
    let { data: existing } = await supabase
      ...
      .select('id, company_name')
      .eq('is_deleted', false)
      .ilike('company_name', importedName.toLowerCase())
      .maybeSingle();
    if (!existing) {
      const { data: exact } = await supabase
        .from('companies')
...
export const modifyCompaniesFromCSV = async (mappedData, userId) => {
  ...
  const uniqueCompanyNames = [...new Set(
    mappedData
      .map(row => row.company_name?.trim().toLowerCase())
      .filter(name => name && name.length > 0)
  )];
...
export const importRepresentativesFromCSV = async (mappedData, userId) => {
  ...
  // After insertion, map imported representative names to their IDs to enrich duplicates
  if (!error && data) {
    const nameToInsertedId = new Map();
    data.forEach(rep => {
      if (rep.full_name && rep.company_id) {
        const key = `${rep.company_id}__${rep.full_name.toLowerCase()}`;
        nameToInsertedId.set(key, rep.id);
      }
    });

    // Enrich duplicates with imported_id
    for (let i = 0; i < duplicates.length; i++) {
      const dup = duplicates[i];
      const companyId = companyNameToIdMap.get(dup.imported_company_name);
      if (companyId && dup.imported_name) {
        const key = `${companyId}__${dup.imported_name.toLowerCase()}`;
        const importedId = nameToInsertedId.get(key);
        if (importedId) {
          duplicates[i] = { ...dup, imported_id: importedId };
        }
      }
    }
  }
