...
export const getCompanies = async (page = 1, limit = 50, filters = {}) => {
  ...
  // Apply deleted filter based on show_deleted flag
  if (!filters.show_deleted) {
    query = query.eq('is_deleted', false); // Only export non-deleted companies
  }
  // If show_deleted is true, we export all records (both deleted and non-deleted)
  // Apply deleted filter based on show_deleted flag
  if (!filters.show_deleted) {
    query = query.eq('is_deleted', false); // Only get non-deleted companies
  }
  // If show_deleted is true, we get all records (both deleted and non-deleted)
...
export const bulkMarkCompaniesReadUnread = async (ids, markUnread, userId) => {
  ...
  if (!error && data) {
    ...
    for (const company of data) {
      const oldCompany = oldData?.find(old => old.id === company.id);
      await logAudit(userId, markUnread ? 'bulk_mark_unread' : 'bulk_mark_read', 'companies', company.id, oldCompany, company);
    }
  }
