...
// Helper function to chunk arrays into smaller pieces to avoid URI too long errors
...
// Helper function to fetch user read status in chunks
const fetchUserReadStatusInChunks = async (userId, entityType, entityIds) => {
  if (!entityIds.length) return [];
  
  const column = entityType === 'company' ? 'company_id' : 'representative_id';
  const chunks = chunkArray(entityIds, 100); // Process 100 IDs at a time
  const allResults = [];
  
  for (const chunk of chunks) {
    const { data } = await supabase
      .from('user_reads')
      .select(`${column}, mark_unread`)
      .eq('user_id', userId)
      .in(column, chunk);
...
export const getCompaniesWithUserReadStatus = async (userId, page = 1, limit = 50, filters = {}) => {
  ...
  // Get read status for all companies in this batch using chunked queries
  const companyIds = data.map(company => company.id);
  const readStatusData = await fetchUserReadStatusInChunks(userId, 'company', companyIds);
...
export const getRepresentativesWithUserReadStatus = async (userId, page = 1, limit = 50, filters = {}) => {
  ...
  if (filters.contacted_by && filters.contacted_by.length > 0) {
    baseQuery = baseQuery.in('contacted_by', filters.contacted_by);
  }
  ...
  // Get read status for all representatives in this batch using chunked queries
  const representativeIds = data.map(rep => rep.id);
  const readStatusData = await fetchUserReadStatusInChunks(userId, 'representative', representativeIds);
...
export const getRepresentativesWithUserReadStatusAndFilteredCount = async (userId, page = 1, limit = 50, filters = {}) => {
  // 1. Build base query with all filters except unread_filter.
  let baseQuery = supabase
    .from('representatives')
    .select('id, mark_unread, company:company_id(company_name)')
    .eq('is_deleted', false);
  ...
  // 2. Fetch all matching representatives IDs and their global read status.
  const { data: allMatchingReps, error: repsError } = await baseQuery;

  if (repsError) {
    return { data: null, count: 0, error: repsError };
  }

  const repIds = allMatchingReps.map(r => r.id);

  // 3. Get user-specific read status for all these representatives using chunked queries.
  const readStatusData = await fetchUserReadStatusInChunks(userId, 'representative', repIds);
  ...
  // 4. Combine global and user-specific status.
  const repsWithFinalReadStatus = allMatchingReps.map(rep => ({
    id: rep.id,
    mark_unread: readStatusMap.has(rep.id)
      ? readStatusMap.get(rep.id)
      : rep.mark_unread
  }));

  // 5. Apply the read status filter.
  let fullyFilteredReps;
  if (filters.unread_filter === 'unread_only') {
    fullyFilteredReps = repsWithFinalReadStatus.filter(r => r.mark_unread);
  } else if (filters.unread_filter === 'read_only') {
    fullyFilteredReps = repsWithFinalReadStatus.filter(r => !r.mark_unread);
  } else {
    fullyFilteredReps = repsWithFinalReadStatus;
  }

  // 6. Now we have the final count.
  const count = fullyFilteredReps.length;

  // 7. Paginate the IDs.
  const from = (page - 1) * limit;
  const to = from + limit - 1;
  const paginatedRepIds = fullyFilteredReps.slice(from, to).map(r => r.id);

  if (paginatedRepIds.length === 0) {
    return { data: [], count, error: null };
  }

  // 8. Fetch full data for the paginated IDs.
  let finalQuery = supabase
    ...
    .select(`
      *,
      ...
      user_exports!left(user_id)
    `)
    .in('id', paginatedRepIds);

  // Apply sorting
  const sortField = filters.sort_field || 'created_at';
  const sortOrder = filters.sort_order === 'asc';
  finalQuery = finalQuery.order(sortField, { ascending: sortOrder });
  if (sortField !== 'created_at') {
  ...
  if (finalError) {
    return { data: null, count: 0, error: finalError };
  }

  // 9. Attach the correct `mark_unread` status to the final data.
  const finalDataWithCorrectReadStatus = finalData.map(rep => ({
    ...rep,
    mark_unread: readStatusMap.has(rep.id)
      ? readStatusMap.get(rep.id)
      : rep.mark_unread // Fallback, though it should be in the map
  }));

  return { data: finalDataWithCorrectReadStatus, count, error: null };
};

export const getCompaniesWithUserReadStatusAndFilteredCount = async (userId, page = 1, limit = 50, filters = {}) => {
  // 1. Build base query with all filters except unread_filter.
  let baseQuery = supabase
    .from('companies')
    .select('id, mark_unread') // Only select what's needed for filtering
    .eq('is_deleted', false);
  ...
  // 2. Fetch all matching company IDs and their global read status.
  const { data: allMatchingCompanies, error: companiesError } = await baseQuery;

  if (companiesError) {
    return { data: null, count: 0, error: companiesError };
  }

  const companyIds = allMatchingCompanies.map(c => c.id);

  // 3. Get user-specific read status for all these companies using chunked queries.
  const readStatusData = await fetchUserReadStatusInChunks(userId, 'company', companyIds);
  ...
  // 4. Combine global and user-specific status.
  const companiesWithFinalReadStatus = allMatchingCompanies.map(company => ({
    id: company.id,
    mark_unread: readStatusMap.has(company.id)
      ? readStatusMap.get(company.id)
      : company.mark_unread
  ...
  // 5. Apply the read status filter.
  let fullyFilteredCompanies;
  if (filters.unread_filter === 'unread_only') {
    fullyFilteredCompanies = companiesWithFinalReadStatus.filter(c => c.mark_unread);
  } else if (filters.unread_filter === 'read_only') {
    fullyFilteredCompanies = companiesWithFinalReadStatus.filter(c => !c.mark_unread);
  } else {
    fullyFilteredCompanies = companiesWithFinalReadStatus;
  }

  // 6. Now we have the final count.
  const count = fullyFilteredCompanies.length;

  // 7. Paginate the IDs.
  const from = (page - 1) * limit;
  const to = from + limit - 1;

  // We need to sort before slicing for pagination to be correct
  // However, we don't have the sorting field here.
  // The sorting will be applied to the final fetch of paginated data.
  // This means the pagination will be on an unsorted list, and then the result will be sorted.
  // For stable pagination, we should ideally sort before slicing.
  // But that would require fetching more data in the first step.
  // We'll proceed with sorting on the final paginated set.

  const paginatedCompanyIds = fullyFilteredCompanies.slice(from, to).map(c => c.id);

  if (paginatedCompanyIds.length === 0) {
    return { data: [], count, error: null };
  }
  ...
  // 9. Attach the correct `mark_unread` status to the final data.
  const finalDataWithCorrectReadStatus = finalData.map(company => ({
    ...company,
    mark_unread: readStatusMap.has(company.id)
      ? readStatusMap.get(company.id)
      : company.mark_unread
