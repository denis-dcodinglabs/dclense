...
export const getRepresentatives = async (page = 1, limit = 50, filters = {}) => {
  ...
  let query = supabase
    ...
    .select(`
      *,
      ...
      user_exports!left(user_id, is_exported)
    `)
    .eq('is_deleted', false); // Only get non-deleted representatives

  // Filter user_exports to only current user's exports
  if (currentUser) {
    query = query.eq('user_exports.user_id', currentUser);
  }
  ...
  if (filters.exported_filter === 'not_exported_only') {
    ...
    if (currentUser) {
      query = query.is('user_exports.user_id', null);
    }
  }
