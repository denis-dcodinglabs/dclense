...
export const exportToCSV = async (data, fields, exportType, purpose, userId, filters = {}) => {
  // Mark representatives as exported for this user if exporting representatives
  if (exportType === 'representatives' && data.length > 0) {
    const { markRepresentativesAsExported } = await import('./representatives');
    const representativeIds = data.map(rep => rep.id);
    await markRepresentativesAsExported(representativeIds, userId);
  }
